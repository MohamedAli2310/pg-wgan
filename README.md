...
The software is an implementation of pg-gan with Wasserstien loss. Below are instructions and detatils of this implementation.
...
- Other experiments (1-3) are not salvagalbe; that is, there is no parameter choice that can make them work for the pg-gan dataset. This is mainly because of how deep the data is and the fact that the experiments use weight clipping, not gradient penality, which prevents the network from capturing more details, as the clipping happens very often, obscuring any information that can be gained from how large the gradient magnitude deviated from the norm.

## The current version

The file pg-wgan.py contatins the current working version of pg-wgan, which is the one that produced Experiment4.out. This version is set as follows:
- 10 proposals per parameter.
- 50 batch size.
- NO PRE-TRAINING.
- 5 Critic epochs per Simulated Annealing step.
- 20,000 Gereator iterations.

## Notes and Future Work:

- As it turns out, pre-training is not necessary at all in pg-wgan. In fact, in some experiments pre-training led to the critic having more control over the learning process and changing the sign it gives to data as it wishes (switching from scoring real data with a large positive and fake data with a large negative to the opposite and vice versa).
- Running the same experiment multiple times should produce similar results.
- Applying a layer normalization layer to the critic archeticture may speed up the entire learning process, but it most likely neither improves nor harms the results.

Please find the original pg-gan README below!
