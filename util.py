...
class Parameter:
    ...
    def __init__(self, value, min, max, name):
        ...
        self.max = max
        self.name = name
        self.proposal_width = (self.max - self.min)/5  # heuristic
    ...
    def proposal(self, curr_value, multiplier):
        ...
        #new_value = curr_value + np.random.randn(self.proposal_width)*multiplier
...
def process_gt_dist(gt_matrix, dist_vec, S, filter=False, rate=None, neg1=True):
    ...
    if filter:
        # mask
        singleton_mask = np.array([filter_func(row, rate)
...
def parse_args():
    ...
    parser.add_option('-g', action="store_true",
                      dest="grid", help='grid search')
    parser.add_option('-t', action="store_true",
                      dest="toy", help='toy example')
